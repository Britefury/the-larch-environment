...
from GSymCore.Languages.Python25.Python25Plugin import py25ImportFile
...
if __name__ == '__main__':
	content = py25ImportFile( 'testsources\\view_web_test_py.py' )
	documentRoot = newDocument( content )

	commandHistory = CommandHistory()
	commandHistory.track( documentRoot )

	contentHandler = GSymDocumentViewContentHandler( commandHistory )
	world = GSymWorld()
	
	def makePage():
		view = loadDocument( world, documentRoot, contentHandler )
		page = view.page
		return page
	
	app = ServerApp.ServerApp( makePage )
