...
#   Portions of code by Geoffrey French <gsculpt at googlemail dot com>.
#
...
def testScriptingWindow(scriptBanner, windowTitle, transientParent=None, bAllowDelete=True, locals={}):
	def onDeleteEvent(window, event):
		return not bAllowDelete

	scriptConsole = Console( locals=locals, banner=scriptBanner, use_rlcompleter=False )
	scriptConsole.show()
	...
	if transientParent is not None:
		scriptWindow.set_transient_for( transientParent )
	scriptWindow.add( scriptScrolledWindow )
	scriptWindow.connect( 'delete-event', onDeleteEvent )
	scriptWindow.set_title( windowTitle )
	scriptWindow.show()

	return scriptWindow, scriptConsole
