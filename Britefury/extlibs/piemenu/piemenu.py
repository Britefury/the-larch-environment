...
class PieMenu(gtk.Window):
    ...
    def layout(self, rect, context, pcontext, playout):
        ...
        if ring_count > 0:
            ...
            for pie_items in pie_rings:
                ...
                ring_clockwise = clockwise[min(len(clockwise) - 1, ring_index)]
                if ring_clockwise:
                    twist = -twist
    ...
    def trackMouseMove(self, cx, cy):
        ...
        if new_item != last_item:
            ...
            if new_item != -1:
                self.items[new_item].enter_time = now

            self.queue_draw()

            # TODO: Signal onchanged event.
	...
	item.activateSignal.emit( self, item )
