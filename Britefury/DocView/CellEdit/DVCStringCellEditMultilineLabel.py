...
from Britefury.DocView.Toolkit.DTEditableMultilineLabel import DTEditableMultilineLabel
from Britefury.DocView.CellEdit.DVCBasicWidgetSingleCellEdit import DVCBasicWidgetSingleCellEdit



class DVCStringCellEditMultilineLabel (DVCBasicWidgetSingleCellEdit):
	"""String cell editor"""
	finishSignal = ClassSignal()		# self, text


	__valueclass__ = str


	def __init__(self, bMarkup=False):
		super( DVCStringCellEditMultilineLabel, self ).__init__()
		self.entry = DTEditableMultilineLabel( bMarkup=bMarkup )
		self.entry.finishEditingSignal.connect( self._p_onFinish )
	...
	def _o_setWidgetValue(self, value):
		...
		if self._bMarkup:
			self.entry.markup = value
		else:
			self.entry.text = value
		self._bIgnoreCheck = False


	def _p_onFinish(self, entry, text):
...
if __name__ == '__main__':
	...
	entry = DVCStringCellEditMultilineLabel()
