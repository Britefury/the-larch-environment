...
class _MemoEntry (object):
	#__slots__ = [ 'answer', 'pos', 'bEvaluating', 'bLeftRecursionDetected', 'lrApplication', 'bGrowingLRParse' ]
	...
	def __init__(self, rule, answer, pos):
		...
		self.lrApplication = None
		self.bGrowingLRParse = False
...
#		if memoEntry is not None  and  memoEntry.lrApplication is not None  and  memoEntry is not memoEntry.lrApplication.memoEntry:
		if memoEntry is not None  and  memoEntry.lrApplication is not None:
			log <=  '__recall %s @ %d; innerLR, lr-application for %s'  %  ( rule.debugName, start, memoEntry.lrApplication.rule.debugName )
			# This rule application is involved in a left-recursive application of a rule
			lrApplication = memoEntry.lrApplication
			...
			elif memoEntry.lrApplication is None:
				log <=  ( '__recall %s @ %d; rule; no lr-application'  %  ( rule.debugName, start ) )
			elif memoEntry is memoEntry.lrApplication.memoEntry is None:
				log <=  ( '__recall %s @ %d; rule; head rule'  %  ( rule.debugName, start ) )
	...
	def __onLeftRecursionDetected(self, rule, input, start, stop, memoEntry):
		...
		if memoEntry.lrApplication is None:
			lrApplication = _LeftRecursiveApplication( memoEntry, rule )
			memoEntry.lrApplication = lrApplication
		else:
			lrApplication = memoEntry.lrApplication
		...
		while invocation is not None   and   invocation.memoEntry is not memoEntry:
			...
			invocation.memoEntry.lrApplication = lrApplication
	...
	def __growLeftRecursiveParse(self, rule, input, start, stop, memoEntry, answer, pos):
		...
		memoEntry.bGrowingLRParse = True
		lrApplication = memoEntry.lrApplication
...
if True:
	...
	class TestCase_ParserState2 (ParserTestCase):
	...
	if __name__ == '__main__':
		...
		if True:
			...
			#ans, pos = methodInvocation.parseString( 'this.m().n()' )
			parser = arrayAccess
			testString = 'this.x[i]'
